...
const App = (() => {
    ...
    function parseQuestionHTML(html, qid) {
        ...
        for (const el of children) {
            ...
            if (el.tagName === 'TABLE') {
                ...
                // Matrix-style choices: rows with single letter (no period) in first cell
                // Require A, B, C at minimum to avoid false positives from data tables
                if (!choicesTable) {
                    const rows = el.querySelectorAll('tr');
                    const foundLetters = new Set();
                    for (const row of rows) {
                        const fc = row.querySelector('td');
                        if (fc && /^[A-F]$/.test(fc.textContent.trim())) {
                            foundLetters.add(fc.textContent.trim());
                        }
                    }
                    if (foundLetters.has('A') && foundLetters.has('B') && foundLetters.has('C')) {
                        choicesTable = el;
                        continue;
                    }
                }
        ...
        if (choicesTable) {
            ...
            rows.forEach((row, ri) => {
                ...
                const firstText = cells[0].textContent.trim().replace('.', '');
                if (/^[A-F]$/.test(firstText)) {
                    const letter = firstText;
                    let text;
                    if (headerLabels.length > 0 && cells.length > 2) {
                        // Matrix format: combine header label + value for each column
                        const parts = [];
                        for (let c = 1; c < cells.length; c++) {
                            const val = cells[c].textContent.trim();
                            if (val) {
                                const label = headerLabels[c - 1] || '';
                    ...
                    // Header row: collect labels (skip first empty/label cell)
                    headerLabels = [...cells].slice(1).map(c => c.textContent.trim()).filter(Boolean);
                }
            });
        }
    ...
    function renderDashboard() {
        ...
        const upgradeBtn = isPaid ? '' : `<button class="upgrade-btn-small" onclick="App.showPaywall()"><svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg> Upgrade</button>`;

        el.innerHTML = `
            <div class="page-header">
                <div class="page-header-top"><h1>Dashboard</h1>${upgradeBtn}</div>
    ...
    async function loadQuestion(idx) {
        ...
        if (!q) {
            ...
            `;
            // Still update buttons so user can navigate away
